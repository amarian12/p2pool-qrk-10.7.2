...
import quark.getwork as quark_getwork, quark.data as quark_data
from quark import helper, script, worker_interface
...
class WorkerBridge(worker_interface.WorkerBridge):
    ...
    def __init__(self, node, my_pubkey_hash, donation_percentage, merged_urls, worker_fee, args, pubkeys, quarkd):
        ...
        def compute_work():
            t = self.node.quarkd_work.value
            bb = self.node.best_block_header.value
            if bb is not None and bb['previous_block'] == t['previous_block'] and self.node.net.PARENT.POW_FUNC(quark_data.block_header_type.pack(bb)) <= t['bits'].target:
                print 'Skipping from block %x to block %x!' % (bb['previous_block'],
                    self.node.net.PARENT.BLOCKHASH_FUNC(quark_data.block_header_type.pack(bb)))
                t = dict(
                    version=bb['version'],
                    previous_block=self.node.net.PARENT.BLOCKHASH_FUNC(quark_data.block_header_type.pack(bb)),
                    ...
                    transactions=[],
                    transaction_fees=[],
                    merkle_link=quark_data.calculate_merkle_link([None], 0),
                    subsidy=self.node.net.PARENT.SUBSIDY_FUNC(self.node.quarkd_work.value['bits'].bits, self.node.quarkd_work.value['height']),
                    last_update=self.node.quarkd_work.value['last_update'],
                    payee=self.node.quarkd_work.value['payee'],
                    payee_amount=self.node.quarkd_work.value['payee_amount'],
                )

            self.current_work.set(t)
        self.node.quarkd_work.changed.watch(lambda _: compute_work())
    ...
    def freshen_addresses(self, c):
        ...
        self.address = yield deferral.retry('Error getting a dynamic address from quarkd:', 5)(lambda: self.quarkd.rpc_getnewaddress('p2pool'))()
        new_pubkey = quark_data.address_to_pubkey_hash(self.address, self.net)
        self.pubkeys.popleft()
        self.pubkeys.addkey(new_pubkey)
        print " Updated payout pool:"
        for i in range(len(self.pubkeys.keys)):
            print '    ...payout %d: %s(%f)' % (i, quark_data.pubkey_hash_to_address(self.pubkeys.keys[i], self.net),self.pubkeys.keyweights[i],)
    ...
    def get_user_details(self, username):
        ...
        for symbol, parameter in zip(contents2[::2], contents2[1::2]):
            if symbol == '+':
                try:
                    desired_pseudoshare_target = quark_data.difficulty_to_target(float(parameter))
                except:
                    if p2pool.DEBUG:
                        log.err()
            elif symbol == '/':
                try:
                    desired_share_target = quark_data.difficulty_to_target(float(parameter))
                except:
                    if p2pool.DEBUG:
                        log.err()        
        ...
        if random.uniform(0, 100) < self.worker_fee:
            ...
            try:
                pubkey_hash = quark_data.address_to_pubkey_hash(user, self.node.net.PARENT)
            except: # XXX blah
                if self.args.address != 'dynamic':
                    pubkey_hash = self.my_pubkey_hash
    ...
    def preprocess_request(self, user):
        ...
        if time.time() > self.current_work.value['last_update'] + 60:
            raise jsonrpc.Error_for_code(-12345)(u'lost contact with quarkd')
    ...
    def get_work(self, pubkey_hash, desired_share_target, desired_pseudoshare_target):
        ...
        if self.merged_work.value:
            tree, size = quark_data.make_auxpow_tree(self.merged_work.value)
            mm_hashes = [self.merged_work.value.get(tree.get(i), dict(hash=0))['hash'] for i in xrange(size)]
            mm_data = '\xfa\xbemm' + quark_data.aux_pow_coinbase_type.pack(dict(
                merkle_root=quark_data.merkle_hash(mm_hashes),
                size=size,
                nonce=0,
            ))
        ...
        tx_hashes = [quark_data.hash256(quark_data.tx_type.pack(tx)) for tx in self.current_work.value['transactions']]
        ...
        if desired_share_target is None:
            ...
            if local_hash_rate is not None:
                desired_share_target = min(desired_share_target,
                    quark_data.average_attempts_to_target(local_hash_rate * self.node.net.SHARE_PERIOD / 0.0167)) # limit to 1.67% of pool shares by modulating share difficulty

            local_addr_rates = self.get_local_addr_rates()
            lookbehind = 3600//self.node.net.SHARE_PERIOD
            block_subsidy = self.node.quarkd_work.value['subsidy']
            if previous_share is not None and self.node.tracker.get_height(previous_share.hash) > lookbehind:
                ...
                if expected_payout_per_block < self.node.net.PARENT.DUST_THRESHOLD:
                    desired_share_target = min(desired_share_target,
                        quark_data.average_attempts_to_target((quark_data.target_to_average_attempts(self.node.quarkd_work.value['bits'].target)*self.node.net.SPREAD)*self.node.net.PARENT.DUST_THRESHOLD/block_subsidy)
                    )
        ...
        packed_gentx = quark_data.tx_type.pack(gentx)
        ...
        if desired_pseudoshare_target is None:
            ...
            if local_hash_rate is not None:
                target = min(target,
                    quark_data.average_attempts_to_target(local_hash_rate * 1)) # limit to 1 share response every second by modulating pseudoshare difficulty
        else:
        ...
        if print_throttle is 0.0:
            ...
            if (current_time - print_throttle) > 5.0:
                print 'New work for worker! Difficulty: %.06f Share difficulty: %.06f Total block value: %.6f %s including %i transactions' % (
                    quark_data.target_to_difficulty(target),
                    quark_data.target_to_difficulty(share_info['bits'].target),
        ...
        #need this for stats
        self.last_work_shares.value[quark_data.pubkey_hash_to_address(pubkey_hash, self.node.net.PARENT)]=share_info['bits']
        ...
        def got_response(header, user, coinbase_nonce):
            ...
            new_gentx = quark_data.tx_type.unpack(new_packed_gentx) if coinbase_nonce != '\0'*self.COINBASE_NONCE_LENGTH else gentx

            header_hash = self.node.net.PARENT.BLOCKHASH_FUNC(quark_data.block_header_type.pack(header))
            pow_hash = self.node.net.PARENT.POW_FUNC(quark_data.block_header_type.pack(header))
            try:
                if pow_hash <= header['bits'].target or p2pool.DEBUG:
                    if self.node.quarkd_work.value['masternode_payments']:
                        helper.submit_block(dict(header=header, txs=[new_gentx] + other_transactions), False, self.node.factory, self.node.quarkd, self.node.quarkd_work, self.node.net)
                    else:
                        helper.submit_block(dict(header=header, txs=[new_gentx] + other_transactions), False, self.node.factory, self.node.quarkd, self.node.quarkd_work, self.node.net)
                    if pow_hash <= header['bits'].target:
                        print
                        print 'GOT BLOCK FROM MINER! Passing to quarkd! %s%064x' % (self.node.net.PARENT.BLOCK_EXPLORER_URL_PREFIX, header_hash)
                        print
            except:
            ...
            assert header['merkle_root'] == quark_data.check_merkle_link(quark_data.hash256(new_packed_gentx), merkle_link)
                ...
                try:
                    if pow_hash <= aux_work['target'] or p2pool.DEBUG:
                        df = deferral.retry('Error submitting merged block: (will retry)', 10, 10)(aux_work['merged_proxy'].rpc_getauxblock)(
                            ...
                            quark_data.aux_pow_type.pack(dict(
                                merkle_tx=dict(
                                    tx=new_gentx,
                                    block_hash=header_hash,
                                    merkle_link=merkle_link,
                                ),
                                merkle_link=quark_data.calculate_merkle_link(hashes, index),
            ...
            if pow_hash <= share_info['bits'].target and header_hash not in received_header_hashes:
                ...
                self.share_received.happened(quark_data.target_to_average_attempts(share.target), not on_time, share.hash)
            ...
            if pow_hash > target:
                ...
                self.pseudoshare_received.happened(quark_data.target_to_average_attempts(target), not on_time, user)
                self.recent_shares_ts_work.append((time.time(), quark_data.target_to_average_attempts(target)))
                while len(self.recent_shares_ts_work) > 50:
                    self.recent_shares_ts_work.pop(0)
                self.local_rate_monitor.add_datum(dict(work=quark_data.target_to_average_attempts(target), dead=not on_time, user=user, share_target=share_info['bits'].target))
                self.local_addr_rate_monitor.add_datum(dict(work=quark_data.target_to_average_attempts(target), pubkey_hash=pubkey_hash))
