...
import quark.p2p as quark_p2p, quark.data as quark_data
from quark import stratum, worker_interface, helper
...
class keypool():
    ...
    def paytotal(self):
        ...
        for i in range(len(pubkeys.keys)):
            self.payouttotal += node.get_current_txouts().get(quark_data.pubkey_hash_to_script2(pubkeys.keys[i]), 0)*1e-5
...
def main(args, net, datadir_path, merged_urls, worker_endpoint):
    try:
        ...
        @defer.inlineCallbacks
        def connect_p2p():
            # connect to quarkd over quark-p2p
            print '''Testing quarkd P2P connection to '%s:%s'...''' % (args.quarkd_address, args.quarkd_p2p_port)
            factory = quark_p2p.ClientFactory(net.PARENT)
            reactor.connectTCP(args.quarkd_address, args.quarkd_p2p_port, factory)
            def long():
                print '''    ...taking a while. Common reasons for this include all of quarkd's connection slots being used...'''
        ...
        if args.testnet: # establish p2p connection first if testnet so quarkd can work without connections
            factory = yield connect_p2p()
        
        # connect to quarkd over JSON-RPC and do initial getmemorypool
        url = '%s://%s:%i/' % ('https' if args.quarkd_rpc_ssl else 'http', args.quarkd_address, args.quarkd_rpc_port)
        print '''Testing quarkd RPC connection to '%s' with username '%s'...''' % (url, args.quarkd_rpc_username)
        quarkd = jsonrpc.HTTPProxy(url, dict(Authorization='Basic ' + base64.b64encode(args.quarkd_rpc_username + ':' + args.quarkd_rpc_password)), timeout=30)
        yield helper.check(quarkd, net)
        temp_work = yield helper.getwork(quarkd, net)
        
        quarkd_getinfo_var = variable.Variable(None)
        @defer.inlineCallbacks
        def poll_warnings():
            quarkd_getinfo_var.set((yield deferral.retry('Error while calling getinfo:')(quarkd.rpc_getinfo)()))
        yield poll_warnings()
        ...
        if args.pubkey_hash is None and args.address != 'dynamic':
            ...
            if address is not None:
                res = yield deferral.retry('Error validating cached address:', 5)(lambda: quarkd.rpc_validateaddress(address))()
                if not res['isvalid'] or not res['ismine']:
                    print '    Cached address is either invalid or not controlled by local quarkd!'
                    address = None
            
            if address is None:
                print '    Getting payout address from quarkd...'
                address = yield deferral.retry('Error getting payout address from quarkd:', 5)(lambda: quarkd.rpc_getaccountaddress('p2pool'))()
            
            with open(address_path, 'wb') as f:
                f.write(address)
            
            my_pubkey_hash = quark_data.address_to_pubkey_hash(address, net.PARENT)
            print '    ...success! Payout address:', quark_data.pubkey_hash_to_address(my_pubkey_hash, net.PARENT)
            print
            ...
            for i in range(args.numaddresses):
                address = yield deferral.retry('Error getting a dynamic address from quarkd:', 5)(lambda: quarkd.rpc_getnewaddress('p2pool'))()
                new_pubkey = quark_data.address_to_pubkey_hash(address, net.PARENT)
            ...
            for i in range(len(pubkeys.keys)):
                print '    ...payout %d: %s' % (i, quark_data.pubkey_hash_to_address(pubkeys.keys[i], net.PARENT),)
        ...
        node = p2pool_node.Node(factory, quarkd, shares.values(), known_verified, net)
        yield node.start()
        ...
        wb = work.WorkerBridge(node, my_pubkey_hash, args.donation_percentage, merged_urls, args.worker_fee, args, pubkeys, quarkd)
        web_root = web.get_web_root(wb, datadir_path, quarkd_getinfo_var)
                ...
                def signedOn(self):
                    ...
                    def new_share(share):
                        ...
                        if share.pow_hash <= share.header['bits'].target and abs(share.timestamp - time.time()) < 10*60:
                            ...
                            message = '\x02%s BLOCK FOUND by %s! %s%064x' % (net.NAME.upper(), quark_data.script2_to_address(share.new_script, net.PARENT), net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash)
        ...
        def status_thread():
            ...
            while True:
                ...
                try:
                    ...
                    my_shares_per_s = sum(datum['work']/dt/quark_data.target_to_average_attempts(datum['share_target']) for datum in datums)
                    ...
                    if height > 2:
                        ...
                        paytot = 0.0
                        for i in range(len(pubkeys.keys)):
                            curtot = node.get_current_txouts().get(quark_data.pubkey_hash_to_script2(pubkeys.keys[i]), 0)
                            paytot += curtot*1e-5
                        ...
                        this_str += '\n Pool: %sH/s Stale rate: %.1f%% Expected time to block: %s' % (
                            ...
                            100*stale_prop,
                            math.format_dt(2**256 / node.quarkd_work.value['bits'].target / real_att_s),
                        )
                        
                        for warning in p2pool_data.get_warnings(node.tracker, node.best_share_var.value, net, quarkd_getinfo_var.value, node.quarkd_work.value):
                            print >>sys.stderr, '#'*40
...
def run():
    ...
    parser.add_argument('--net',
        help='use specified network (default: quark)',
        action='store', choices=sorted(realnets), default='quark', dest='net_name')
    ...
    parser.add_argument('-a', '--address',
        help='generate payouts to this address (default: <address requested from quarkd>), or (dynamic)',
        type=str, action='store', default=None, dest='address')
    parser.add_argument('-i', '--numaddresses',
        help='number of quark auto-generated addresses to maintain for getwork dynamic address allocation',
    ...
    worker_group.add_argument('-f', '--fee', metavar='FEE_PERCENTAGE',
        help='''charge workers mining to their own quark address (by setting their miner's username to a quark address) this percentage fee to mine on your p2pool instance. Amount displayed at http://127.0.0.1:WORKER_PORT/fee (default: 0)''',
        type=float, action='store', default=0, dest='worker_fee')
    
    quarkd_group = parser.add_argument_group('quarkd interface')
    quarkd_group.add_argument('--quarkd-config-path', metavar='QUARKD_CONFIG_PATH',
        help='custom configuration file path (when quarkd -conf option used)',
        type=str, action='store', default=None, dest='quarkd_config_path')
    quarkd_group.add_argument('--quarkd-address', metavar='QUARKD_ADDRESS',
        help='connect to this address (default: 127.0.0.1)',
        type=str, action='store', default='127.0.0.1', dest='quarkd_address')
    quarkd_group.add_argument('--quarkd-rpc-port', metavar='QUARKD_RPC_PORT',
        help='''connect to JSON-RPC interface at this port (default: %s <read from quark.conf if password not provided>)''' % ', '.join('%s:%i' % (name, net.PARENT.RPC_PORT) for name, net in sorted(realnets.items())),
        type=int, action='store', default=None, dest='quarkd_rpc_port')
    quarkd_group.add_argument('--quarkd-rpc-ssl',
        help='connect to JSON-RPC interface using SSL',
        action='store_true', default=False, dest='quarkd_rpc_ssl')
    quarkd_group.add_argument('--quarkd-p2p-port', metavar='QUARKD_P2P_PORT',
        help='''connect to P2P interface at this port (default: %s <read from quark.conf if password not provided>)''' % ', '.join('%s:%i' % (name, net.PARENT.P2P_PORT) for name, net in sorted(realnets.items())),
        type=int, action='store', default=None, dest='quarkd_p2p_port')
    
    quarkd_group.add_argument(metavar='QUARKD_RPCUSERPASS',
        help='quarkd RPC interface username, then password, space-separated (only one being provided will cause the username to default to being empty, and none will cause P2Pool to read them from quark.conf)',
        type=str, action='store', default=[], nargs='*', dest='quarkd_rpc_userpass')
    ...
    if len(args.quarkd_rpc_userpass) > 2:
        parser.error('a maximum of two arguments are allowed')
    args.quarkd_rpc_username, args.quarkd_rpc_password = ([None, None] + args.quarkd_rpc_userpass)[-2:]
    
    if args.quarkd_rpc_password is None:
        conf_path = args.quarkd_config_path or net.PARENT.CONF_FILE_FUNC()
        if not os.path.exists(conf_path):
            parser.error('''quark configuration file not found. Manually enter your RPC password.\r\n'''
                '''If you actually haven't created a configuration file, you should create one at %s with the text:\r\n'''
                '''\r\n'''
                '''server=1\r\n'''
                '''rpcpassword=%x\r\n'''
                '''\r\n'''
                '''Keep that password secret! After creating the file, restart quark.''' % (conf_path, random.randrange(2**128)))
        ...
        for conf_name, var_name, var_type in [
            ('rpcuser', 'quarkd_rpc_username', str),
            ('rpcpassword', 'quarkd_rpc_password', str),
            ('rpcport', 'quarkd_rpc_port', int),
            ('port', 'quarkd_p2p_port', int),
        ]:
        ...
        if 'rpcssl' in contents and contents['rpcssl'] != '0':
                args.quarkd_rpc_ssl = True
        if args.quarkd_rpc_password is None:
            parser.error('''quark configuration file didn't contain an rpcpassword= line! Add one!''')
    
    if args.quarkd_rpc_username is None:
        args.quarkd_rpc_username = ''
    
    if args.quarkd_rpc_port is None:
        args.quarkd_rpc_port = net.PARENT.RPC_PORT
    
    if args.quarkd_p2p_port is None:
        args.quarkd_p2p_port = net.PARENT.P2P_PORT
    ...
    if args.address is not None and args.address != 'dynamic':
        try:
            args.pubkey_hash = quark_data.address_to_pubkey_hash(args.address, net.PARENT)
        except Exception, e:
