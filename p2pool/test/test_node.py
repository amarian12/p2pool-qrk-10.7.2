...
from p2pool.quark import data as quark_data, networks, worker_interface
from p2pool.util import deferral, jsonrpc, math, variable

class quarkd(object): # can be used as p2p factory, p2p protocol, or rpc jsonrpc proxy
    def __init__(self):
        ...
        self.headers = {0x16c169477c25421250ec5d32cf9c6d38538b5de970a2355fd89: {
            ...
            'bits': quark_data.FloatingInteger(bits=0x1a0513c5, target=0x513c50000000000000000000000000000000000000000000000L),
        }}
    ...
    def rpc_getblocktemplate(self, param):
        if param['mode'] == 'template':
            ...
            result = param['data']
            block = quark_data.block_type.unpack(result.decode('hex'))
            ...
            if block['header']['previous_block'] != self.blocks[-1]:
                return False
            if quark_data.hash256(result.decode('hex')) > block['header']['bits'].target:
                return False
            header_hash = quark_data.hash256(quark_data.block_header_type.pack(block['header']))
        ...
        for i in xrange(100):
            fee = i
            txs.append(dict(
                data=quark_data.tx_type.pack(dict(version=1, tx_ins=[], tx_outs=[dict(value=fee, script='hello!'*100)], lock_time=0)).encode('hex'),
                fee=fee,
            ))
        return {
            "version" : 3,
...
class MiniNode(object):
    ...
    def start(cls, net, factory, quarkd, peer_ports, merged_urls):
        self = cls()
        
        self.n = node.Node(factory, quarkd, [], [], net)
        yield self.n.start()
