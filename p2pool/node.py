...
from p2pool.quark import data as quark_data, helper, height_tracker
...
class P2PNode(p2p.Node):
    ...
    def handle_shares(self, shares, peer):
        ...
        for share, new_txs in shares:
            if new_txs is not None:
                all_new_txs.update((quark_data.hash256(quark_data.tx_type.pack(new_tx)), new_tx) for new_tx in new_txs)
    ...
    def handle_bestblock(self, header, peer):
        if self.node.net.PARENT.POW_FUNC(quark_data.block_header_type.pack(header)) > header['bits'].target:
        ...
        def _(share):
            ...
            def spread():
                if (self.node.get_height_rel_highest(share.header['previous_block']) > -5 or
                    self.node.quarkd_work.value['previous_block'] in [share.header['previous_block'], share.header_hash]):
...
class Node(object):
    def __init__(self, factory, quarkd, shares, known_verified_share_hashes, net):
    ...
    def start(self):
        ...
        self.quarkd_work = variable.Variable((yield helper.getwork(self.quarkd, self.net)))
        @defer.inlineCallbacks
        def work_poller():
            while stop_signal.times == 0:
                flag = self.factory.new_block.get_deferred()
                try:
                    self.quarkd_work.set((yield helper.getwork(self.quarkd, self.net, self.quarkd_work.value['use_getblocktemplate'])))
                except:
                    log.err()
        ...
        def handle_header(new_header):
            ...
            if not (self.net.PARENT.POW_FUNC(quark_data.block_header_type.pack(new_header)) <= self.quarkd_work.value['bits'].target):
                return
            quarkd_best_block = self.quarkd_work.value['previous_block']
            if (self.best_block_header.value is None
                or (
                    new_header['previous_block'] == quarkd_best_block and
                    self.net.PARENT.BLOCKHASH_FUNC(quark_data.block_header_type.pack(self.best_block_header.value)) == quarkd_best_block
                ) # new is child of current and previous is current
                or (
                    self.net.PARENT.BLOCKHASH_FUNC(quark_data.block_header_type.pack(new_header)) == quarkd_best_block and
                    self.best_block_header.value['previous_block'] != quarkd_best_block
        ...
        def poll_header():
            if self.factory.conn.value is None:
                return
            handle_header((yield self.factory.conn.value.get_block_header(self.quarkd_work.value['previous_block'])))
        self.quarkd_work.changed.watch(lambda _: poll_header())
        ...
        self.get_height_rel_highest = yield height_tracker.get_height_rel_highest_func(self.quarkd, self.factory, lambda: self.quarkd_work.value['previous_block'], self.net)
        
        self.best_share_var = variable.Variable(None)
        self.desired_var = variable.Variable(None)
        self.quarkd_work.changed.watch(lambda _: self.set_best_share())
        self.set_best_share()
        
        # setup p2p logic and join p2pool network
        
        # update mining_txs according to getwork results
        @self.quarkd_work.changed.run_and_watch
        def _(_=None):
            new_mining_txs = {}
            new_known_txs = dict(self.known_txs_var.value)
            for tx_hash, tx in zip(self.quarkd_work.value['transaction_hashes'], self.quarkd_work.value['transactions']):
                new_mining_txs[tx_hash] = tx
                new_known_txs[tx_hash] = tx
            self.mining_txs_var.set(new_mining_txs)
            self.known_txs_var.set(new_known_txs)
        # add p2p transactions from quarkd to known_txs
        @self.factory.new_tx.watch
        def _(tx):
            new_known_txs = dict(self.known_txs_var.value)
            new_known_txs[quark_data.hash256(quark_data.tx_type.pack(tx))] = tx
            self.known_txs_var.set(new_known_txs)
        # forward transactions seen to quarkd
        ...
        def _(share):
            ...
            if block is None:
                print >>sys.stderr, 'GOT INCOMPLETE BLOCK FROM PEER! %s quark: %s%064x' % (p2pool_data.format_hash(share.hash), self.net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash)
                return
            helper.submit_block(block, True, self.factory, self.quarkd, self.quarkd_work, self.net)
            print
            print 'GOT BLOCK FROM PEER! Passing to quarkd! %s quark: %s%064x' % (p2pool_data.format_hash(share.hash), self.net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash)
            print
