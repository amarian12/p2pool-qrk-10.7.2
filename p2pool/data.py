...
import p2pool
from p2pool.quark import data as quark_data, script, sha256
...
class Share(object):
    ...
    small_block_header_type = pack.ComposedType([
        ...
        ('bits', quark_data.FloatingIntegerType()),
    ...
    share_info_type = pack.ComposedType([
        ...
        ('max_bits', quark_data.FloatingIntegerType()),
        ('bits', quark_data.FloatingIntegerType()),
        ('timestamp', pack.IntType(32)),
    ...
    def get_ref_hash(cls, net, share_info, ref_merkle_link):
        return pack.IntType(256).pack(quark_data.check_merkle_link(quark_data.hash256(cls.ref_type.pack(dict(
    ...
    def __init__(self, net, peer_addr, contents):
        ...
        self.new_script = quark_data.pubkey_hash_to_script2(self.share_data['pubkey_hash'])
        ...
        merkle_root = quark_data.check_merkle_link(self.gentx_hash, self.merkle_link)
        self.header = dict(self.min_header, merkle_root=merkle_root)
        self.pow_hash = net.PARENT.POW_FUNC(quark_data.block_header_type.pack(self.header))
        self.hash = self.header_hash = net.PARENT.BLOCKHASH_FUNC(quark_data.block_header_type.pack(self.header))
    ...
    def check(self, tracker):
        ...
        if quark_data.hash256(quark_data.tx_type.pack(gentx)) != self.gentx_hash:
            raise ValueError('''gentx doesn't match hash_link''')
        
        if quark_data.calculate_merkle_link([None] + other_tx_hashes, 0) != self.merkle_link:
    ...
    def should_punish_reason(self, previous_block, bits, tracker, known_txs):
        ...
        if other_txs is None:
            ...
            all_txs_size = sum(quark_data.tx_type.packed_size(tx) for tx in other_txs)
            if all_txs_size > 1000000:
                return True, 'txs over block size limit'
            
            new_txs_size = sum(quark_data.tx_type.packed_size(known_txs[tx_hash]) for tx_hash in self.share_info['new_transaction_hashes'])
...
class OkayTracker(forest.Tracker):
    ...
    def think(self, block_rel_height_func, previous_block, bits, known_txs):
        ...
        if p2pool.DEBUG:
            print 'Desire %i shares. Cutoff: %s old diff>%.2f' % (len(desired), math.format_dt(time.time() - timestamp_cutoff), quark_data.target_to_difficulty(target_cutoff))
            for peer_addr, hash, ts, targ in desired:
                print '   ', None if peer_addr is None else '%s:%i' % peer_addr, format_hash(hash), math.format_dt(time.time() - ts), quark_data.target_to_difficulty(targ), ts >= timestamp_cutoff, targ <= target_cutoff
...
def get_warnings(tracker, best_share, net, quarkd_getinfo, quarkd_work_value):
    res = []
    ...
    if quarkd_getinfo['errors'] != '':
        if 'This is a pre-release test build' not in quarkd_getinfo['errors']:
            res.append('(from quarkd) %s' % (quarkd_getinfo['errors'],))
    
    version_warning = getattr(net, 'VERSION_WARNING', lambda v: None)(quarkd_getinfo['version'])
    if version_warning is not None:
        res.append(version_warning)
    
    if time.time() > quarkd_work_value['last_update'] + 60:
        res.append('''LOST CONTACT WITH QUARKD for %s! Check that it isn't frozen or dead!''' % (math.format_dt(time.time() - quarkd_work_value['last_update']),))
