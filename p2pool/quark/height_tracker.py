...
class HeaderWrapper(object):
    ...
    def from_header(cls, header):
        return cls(quark_data.hash256(quark_data.block_header_type.pack(header)), header['previous_block'])
...
@defer.inlineCallbacks
def get_height_rel_highest_func(quarkd, factory, best_block_func, net):
    if '\ngetblock ' in (yield deferral.retry()(quarkd.rpc_help)()):
        @deferral.DeferredCacher
        @defer.inlineCallbacks
        def height_cacher(block_hash):
            try:
                x = yield quarkd.rpc_getblock('%x' % (block_hash,))
